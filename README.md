...
- `dist/learn/`: First Piano Journey `/learn` surface — First Notes unit (L01–L04). ES modules, no build step.
...
Open `http://localhost:3000` for the marketing page, `http://localhost:3000/learn/` for the First Notes unit, and `http://localhost:3000/learn/?lesson=L01` for Meet the keyboard. No dependency installation or build step is needed.
...
`/learn` is a separate First Piano Journey surface. First Notes (L01–L04) saves versioned attempt records in `localStorage` under `meetpiano:beginner-v1` on this device only. The next activity unlocks when this device is ready. Demo playback does not earn progress. There are no accounts and no cloud sync. Physical MIDI hardware is not claimed as verified.

...
- `dist/learn/`: First Piano Journey `/learn` surface — First Notes (L01–L04), Rhythm Club (L05–L08), Read and play (L09–L12), and Left hand (L13–L16). ES modules, no build step.
...
Open `http://localhost:3000` for the marketing page, `http://localhost:3000/learn/` for the journey hub, `http://localhost:3000/learn/?lesson=L01` for Meet the keyboard, `http://localhost:3000/learn/?unit=rhythm-club` for Rhythm Club, `http://localhost:3000/learn/?unit=read-and-play` for Read and play, and `http://localhost:3000/learn/?unit=left-hand` for Left hand. No dependency installation or build step is needed.
...
`/learn` is a separate First Piano Journey surface. First Notes (L01–L04), Rhythm Club (L05–L08), Read and play (L09–L12), and Left hand (L13–L16) save versioned attempt records in `localStorage` under `meetpiano:beginner-v1` on this device only. Rhythm scoring uses a shared audio clock, not the screen animation. Staff pitch, clef, duration, and expected keys agree. MIDI reports pitch and time only; it does not certify which hand. The next activity unlocks when this device is ready. Demo playback does not earn progress. There are no accounts and no cloud sync. Physical MIDI hardware is not claimed as verified.
